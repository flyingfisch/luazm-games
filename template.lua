...
while exit~=1 do
	--wait for key
	while not keyMenuFast() do end
	print(answers[math.random(1,20)])
	print("again? (F1: yes, any other key: no)")
	
	while not keyMenuFast() do key1=keyMenuFast end
	if key1=key.F1 then exit=0 else exit=1 print("Hey! I'm back, ask a question, press a key, what's bad for you is good for me.")
end
